...
先通过 启动模版并且使用 `vscode debugger` 断点步入到源码中，之后在 `vm._update(vm._render(), hydrating)` 处进行断点。
...
- 如果是第一次初始化的流程，由于第一个 `oldVnode` 是一个真实的 DOM 因此 会走 `oldVnode = emptyNodeAt(oldVnode)` 将这个真实的 DOM 映射成一个 虚拟 DOM
...
这里解释一下 根组件挂载与普通的组件创建为什么走的是不一样的流程：
...
> 这里进行的 `updateChildren` 将子节点对比也是调用的 `patchVnode`，递归到最后也就是判断 `文本节点/空节点` 的 增删问题
